...
int nthreads;
scene_t scene;
//gloabl array to hold pixel structs
pixel global_pix[width][height];

//function pointer used for multithreading.  Pointer so it can be referenced
//in our pthread function.  This is the main bulk of computation previously in
//int main, but now we use it as a function to parallelize the pixel computation.
void * pixelCalulation(void *pid)
{
  ...
  /* for every pixel with the correct column */
  for (int px = *(int*)pid; px<width; px+=nthreads)
  {
        ...
        for (int ys = -halfSamples; ys <= halfSamples; ++ys)
	{
	  ...
	  /* trace the ray from the camera that passes through this pixel */
      ...
      /* at this point, have accumulated (2*halfSamples)^2 samples, so need 
       * to average out the final pixel color 
       */
      if (halfSamples)
      {
        mul(pixel_color, pixel_color,(1.0 / (4.0 * halfSamples * halfSamples)));
      }
      ...
      //add the pixel to the global pixel array
      pixel p;
      p.one = scaled_color[0];
      p.two = scaled_color[1];
      p.three = scaled_color[2];
      global_pix[px][py] = p;
    }
  }
  return NULL;
...
main( int argc, char **argv )
{
    ...
    //remove now that we implemented multithreading
    //if( nthreads != 1 )
    //{
    ...
    //my addition
    //create threads to run the pixelCalculation in parallel

    //array of threads and their pids
    ...
    for (i = 0; i < nthreads; i++){
      //assign each thread an ID
      thread_id[i] = i;
      //create the thread, if it fails return an error
      if (pthread_create(&thread[i], NULL, pixelCalulation, &thread_id[i])){
    ...
    //print out each pixel in the global_pix array
    for (i = 0; i < width; i++){
      for (j = 0; j < height; j++){
	printf("%.0f %.0f %.0f\n", global_pix[i][j].one, global_pix[i][j].two, global_pix[i][j].three);
      }
      printf("\n");
    }
